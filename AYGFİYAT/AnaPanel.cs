    ...
    public partial class AnaPanel : Form
    {
        ...
        private void GetExchangeRates()
        {
            ...
            foreach (HtmlElement name in htmlElementCollection)
            {
                ...
                {
                    dolar = Convert.ToDecimal( name.InnerText);
                    label8.Text = name.InnerText;
                }
                else if (name.GetAttribute("className") == "value euro-bid")
                {
                    euro = Convert.ToDecimal(name.InnerText);
                    label7.Text = name.InnerText;
                }
                else if (name.GetAttribute("className") == "value eur-usd-bid")
                {
        ...
        private void printDocument1_PrintPage(object sender, PrintPageEventArgs e)
        {
            try
            {
                ...
                Image image = Image.FromFile("C:\\RESİM\\ayg1.png");
